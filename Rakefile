...
notebooks = FileList["notebooks/*.ipynb"]
notebooks_output = notebooks.map do |path|
  path.gsub(/^notebooks\//, "_posts/").gsub(/\.ipynb/, ".md")
end

task :notebooks => notebooks_output

# rule '.md' => '.ipynb' do |task|
rule '.md' => ->(t){t.pathmap("notebooks/%f").ext(".ipynb")} do |task|
  src = File.absolute_path(task.source)
  dest = File.absolute_path(task.name)
  Dir.chdir("scripts/jupyter") do
    sh "./run_nbconvert.sh #{src} #{dest}"
  end
end
